...
class QLearning:
    ...
    def run_episode(self, q_table, epsilon):
        ...
        for _ in range(self.timesteps):
            ...
            q_table[state, action] += self.learning_rate * (reward + self.discount_factor * np.max(q_table[next_state]) - q_table[state, action])
            total_reward += reward
    ...
    def run(self, learning_rate=0.15, epsilon_start=1.0, epsilon_min=0.01, epsilon_decay=0.995):
        ...
        qlearning_cumulative_rewards = []  # To store cumulative rewards for SARSA
        all_epsilon_values = []
        ...
        for _ in range(100):  # Run 100 trials
            ...
            qlearning_cumulative_rewards.append(cumulative_rewards)
        ...
        mean_qlearning_cumulative_rewards = np.mean(qlearning_cumulative_rewards, axis=0)
        ...
        ax1.errorbar(episodes, mean_rewards, yerr=std_rewards, label='Q-Learning', fmt='-o', color='r', ecolor='k', capsize=3)
        ...
        fig.tight_layout()  
        plt.title('Mean Rewards and Epsilon Decay (Q-Learning)')
        plt.show()
        
        # Return the mean cumulative rewards for further analysis
        return mean_qlearning_cumulative_rewards
...
if __name__ == "__main__":
    ...
    mean_qlearning_cumulative_rewards = qlearning.run(learning_rate=0.1, epsilon_start=0.5)
