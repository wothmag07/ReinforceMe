...
class SARSA_Lambda:
    ...
    def run_episode(self, q_table, epsilon):
        ...
        for _ in range(self.timesteps):
            ...
            td_error = reward + self.discount_factor * q_table[next_state, next_action] - q_table[state, action]
    ...
    def run(self, learning_rate=0.15, epsilon_start=1.0, epsilon_min=0.01, epsilon_decay=0.995):
        ...
        plt.title('Average Rewards per Episode with Standard Deviation - SARSA_Lambda')
        plt.xlabel('Episode')
...
sarsaAgent = SARSA_Lambda(grid_world)
mean_sarsa_rewards = sarsaAgent.run(learning_rate=0.15, epsilon_start=0.1)
